...
export default async function CalendarioPage({
  ...
  if (calendarConfigured()) {
    // Prefer detailed events (title/attendees/Meet link for the click
    // popover); fall back to plain freebusy intervals if it fails.
    ocupadosInfo = await listar_ocupados_detalle(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-foreground text-2xl font-bold">Calendario</h1>
      ...
      <div className="border-border bg-card overflow-x-auto rounded-xl border">
        <div className="min-w-[760px]">
          {/* Day headers */}
          <div className="border-border grid grid-cols-[64px_repeat(7,minmax(0,1fr))] border-b">
            <div className="text-muted-foreground px-2 py-2 text-right text-[10px] font-semibold tracking-wider uppercase">
              Hora
            </div>
            {dias.map((d, i) => (
              <div
                key={d.clave}
                className="border-border flex items-center justify-center gap-2 border-l py-2"
              >
          ...
          <div className="relative grid grid-cols-[64px_repeat(7,minmax(0,1fr))]">
            <div className="flex flex-col">
              {HOURS.map((h) => (
                <div
                  key={h}
                  className="text-muted-foreground h-[60px] pt-1 pr-2 text-right text-[10px] tabular-nums"
                >
            ...
            {dias.map((d) => {
              ...
              return (
                <div
                  key={d.clave}
                  className="cal-horas border-border relative border-l"
                ...
                >
                  ...
                  {abierto &&
                    shadeBottom < FIN_LABORAL_MIN - INICIO_LABORAL_MIN && (
                      <div
                  ...
                  {d.clave === hoyKey &&
                    lineaTop >= 0 &&
                    lineaTop <= DURACION_LABORAL_MIN && (
                      <div
                        className="cal-linea-ahora"
                        style={{ top: lineaTop }}
                      />
                    )}
                </div>
      ...
      <div className="text-muted-foreground flex flex-wrap items-center gap-x-5 gap-y-2 text-xs">
        ...
        <span className="flex items-center gap-1.5">
          <span className="border-border bg-card size-2.5 rounded-sm border" />
          Disponible
        </span>
      </div>
    </div>
