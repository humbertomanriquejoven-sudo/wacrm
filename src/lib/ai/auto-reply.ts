...
/**
 * Neutral, non-committal reply used when the model wrote a booking
 * confirmation (date + fake Meet/calendar URL) without a real
 * agendar_cita success. Never let a hallucinated "agendada" reach the
 * customer — say we are registering it and the real confirmation will
 * follow.
 */
export const AGENDAR_RETRY_MESSAGE =
  'Un momento, por favor: estoy registrando tu cita en el calendario. Enseguida te confirmo la fecha, la hora y el enlace de tu reunión.'
 ...
 * message BEFORE it is sent to the customer:
 *  - real agendar_cita success + link  → replace every Meet/calendar URL
 *    in the text with that real link (and append it if the model omitted it),
 *  - real success but no link         → strip every Meet/calendar URL,
 *  - no real success this turn        → a booking claim (with or without a
 *    URL) is replaced by AGENDAR_RETRY_MESSAGE; other text just gets its
 *    fake URLs stripped.
 */
export function guardBookingReply(
  ...
  const stripped = text.replace(FAKE_LINK_RE, '')
  return confirmed || !looksLikeBookingConfirmation(text)
    ? stripped
...
export async function dispatchInboundToAiReply(
  ...
  try {
    ...
    // returned link; a booking claim without a real success never goes out.
    if (finalText && !handoff) {
