...
export function buildSystemPrompt(args: {
  ...
  const parts: string[] = [
    ...
    'Continuity and fragmented messages: treat the whole conversation as a single thread. Short, partial, or consecutive messages from the same customer ("Hola", "???", "Hola?", "Estás ahí?") are part of the same message — group their meaning with the recent history and reply once, never restarting the flow or opening a new conversation. ' +
      'Never stay silent and never return an empty reply. If the customer only sent a greeting or short filler ("Hola", "???", a single dot, or a single character), answer politely and professionally, immediately offering help (for example, greet back and ask what you can help them with today).',
    'Contact identification: every interaction happens on the profile of the client who is active in this conversation. You are always speaking to that same person — never treat a new message as a new/simulated user and never invent a different client. If the customer gives you their own details (name, email, city, project, budget), save or update them in their existing profile with the update_client_profile tool; do not suggest, request, or create a separate profile. Assume continuity of one single client per conversation from the first message onward.',
